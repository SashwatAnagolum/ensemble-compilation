...
module {
    ...
    func.func @main(%qubits: tensor<16 x !ensemble.physical_qubit>, %cbits: tensor<16 x !ensemble.cbit>, %circuit_index : i32)  {
        ...
        %phase_gate_powers = ensemble.int_uniform %zero, %four, [%four, %sixteen] : (i32, i32, i32, i32) -> tensor<4 x 16 xi32> 
        ...
        ensemble.reset_tensor %qubits : (tensor<16 x !ensemble.physical_qubit>) -> ()
        
        affine.for %i = 0 to 16 step 1 {
            // gate1q "H" qubits[i];
            %1 = tensor.extract %qubits[%i] : tensor<16 x !ensemble.physical_qubit>
            ensemble.apply %h_gate %1 : (!ensemble.gate, !ensemble.physical_qubit) -> ()
        }
        
        affine.for %i = 0 to 15 step 1 {
            // %qubits[i]
            %qubit_at_i = tensor.extract %qubits[%i] : tensor<16 x !ensemble.physical_qubit> // qubit[i]
            %one = arith.constant 1 : index  
            %iplus1 = arith.addi %i, %one : index  // i + 1
            // %qubits[i+1]
            %qubit_at_i_plus_1 = tensor.extract %qubits[%iplus1] : tensor<16 x !ensemble.physical_qubit> // qubit[i+1]
            ...
            %phase_gate_powers_0_i = tensor.extract %phase_gate_powers[%zero_index, %i] : tensor<4 x 16 xi32>
